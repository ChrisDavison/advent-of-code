...
def display_grid(cheat):
    for r in range(rows):
        for c in range(cols):
            # display cheat
            if (
                (cheat[0] <= r <= cheat[2] or cheat[2] <= r <= cheat[0])
...
while current_cell != end:
    ...
    if c - 1 >= 0 and grid[r, c - 1] == PATH and (r, c - 1) not in path:
        path.append((r, c - 1))
    elif c + 1 < cols and grid[r, c + 1] == PATH and (r, c + 1) not in path:
        path.append((r, c + 1))
    elif r - 1 >= 0 and grid[r - 1, c] == PATH and (r - 1, c) not in path:
        path.append((r - 1, c))
    else:
        path.append((r + 1, c))
    current_cell = path[-1]
    ...
    for r2 in range(-20, 21):
        for c2 in range(-20 + abs(r2), 21 - abs(r2)):
            ...
            if (
                ...
                cheats[(r1, c1, r1 + r2, c1 + c2)] = (
                    grid[r1 + r2, c1 + c2] - grid[r1, c1] - abs(r2) - abs(c2)
                )
