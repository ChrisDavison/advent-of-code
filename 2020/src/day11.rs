...
fn should_swap_p1(data: &Vec<Vec<u8>>, i: usize, j: usize) -> bool {
...
fn should_swap_p2(data: &Vec<Vec<u8>>, i: usize, j: usize) -> bool {
    let mut neighbours = DIRS
        .iter()
        .filter_map(|&direc| find_neighbour(&data, (i, j), direc));
    match data[i][j] {
...
fn iterate_till_stable<F: Fn(&Vec<Vec<u8>>, usize, usize) -> bool>(
    data: Vec<Vec<u8>>,
    swap_checker: F,
) -> usize {
    let mut data = data;
...
#[allow(dead_code)]
fn bytegrid_to_string(grid: &Vec<Vec<u8>>) -> String {
    grid.iter()
